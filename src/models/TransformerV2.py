...
class TransformerV2(nn.Module):
    def __init__(self, args) -> None:
        ...
        for loc in self.locations:
            ...
            module_list = [
                nn.Linear(self.config["freq_out_channels"], self.config["loc_mod_out_channels"]),
                # PositionalEncoding(self.config["loc_mod_out_channels"], 0.1, len(self.modalities)),
            ] + [
                TransformerEncoderLayer(
        ...
        module_list = [
            nn.Linear(self.config["loc_mod_out_channels"], self.config["loc_out_channels"]),
            # PositionalEncoding(self.config["loc_out_channels"], 0.1, len(self.locations)),
        ] + [
            TransformerEncoderLayer(
        ...
        # Single sample, [b, c]
        module_list = [
            nn.Linear(self.config["loc_out_channels"], self.config["interval_out_channels"]),
        ...
        self.interval_context_layer = nn.Sequential(*module_list)
        self.interval_fusion_layer = TransformerFusionBlock(
    ...
    def forward(self, freq_x, class_head=True):
        ...
        for loc in self.locations:
            ...
            for mod in self.modalities:
                """(loc, mod, freq) feature extraction"""
                # [b, c, i, spectrum] -- > [b, i, spectrum, c]
                loc_mod_freq_input = torch.permute(freq_x[loc][mod], [0, 2, 3, 1])
                b, i, s, c = loc_mod_freq_input.shape
                stride = self.config["in_stride"][mod]
                loc_mod_freq_input = torch.reshape(loc_mod_freq_input, (b * i, -1, stride * c))
                loc_mod_freq_context_feature = self.freq_context_layers[loc][mod](loc_mod_freq_input)
                loc_mod_freq_context_feature = loc_mod_freq_context_feature.reshape(
                    [b, i, int(s / stride), self.config["freq_out_channels"]]
                )

                """freq feature fusion, [b, i, 1, c]"""
                loc_mod_feature = self.freq_fusion_layers[loc][mod](loc_mod_freq_context_feature)
                loc_mod_features[loc].append(loc_mod_feature)
        ...
        if len(self.locations) > 1:
            ...
            interval_features = self.loc_fusion_layer(loc_context_feature)
        else:
