...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)
        # changed for collecting local attention weights
        attn_weights = attn.detach() # accumulate attention weights for later use
        # continue regular
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows, attn_weights = self.attn(x_windows, mask=self.attn_mask)  # nW*B, window_size*window_size, C # torch.Size([4096, 8, 64])
        if False:
            # FALSE for now, figure out collect attention weights for a full epoch
...
class BasicLayer(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    shift_size=[0, 0] if (i % 2 == 0) else [1, 0],  # TODO: not sure
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop,
    ...
    def forward(self, x): # x= B, L, C = B, H*W, C = (128, 512, 64)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape # torch.Size([128, 4, 8, 256])
        # FIXME look at relaxing size constraints
        assert (
            H == self.img_size[0] and W == self.img_size[1]
        ), f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."

        x = self.proj(x)  # B Ph*Pw C # torch.Size([128, 64, 8, 32])
        x = x.flatten(2).transpose(1, 2) # B Ph*Pw C # torch.Size([128, 256, 64])
