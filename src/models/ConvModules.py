...
class FIC(nn.Module):
    ...
    def forward(self, x):
        # x: B * C * F * L
        B, C, F = x.size(0), x.size(1), x.size(2)

        x = torch.reshape(x, (B * C * F, -1)).unsqueeze(1)
        x = self.conv(x)
        x = torch.reshape(x, (B, C, F, -1))

        # concatenate x to itself at channel dimension -- TEMPORARY for shape compatibility
