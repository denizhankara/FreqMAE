    ...
    if args.option == "train":
        if args.train_mode == "supervised":
            ...
            if args.stage == "pretrain":
                ...
                # check if the log file exists, and create a new one with _1, _2, etc.
                if os.path.exists(args.train_log_file):
                    i = 2 # start from 2 because the original log file is already created
                    while os.path.exists(args.train_log_file):
                        args.train_log_file = os.path.join(
                            weight_folder, f"{args.task}_{args.label_ratio}_{args.stage}_log_{i}.txt"
                        )
                        args.tensorboard_log = os.path.join(
                            weight_folder, f"{args.task}_{args.label_ratio}_{args.stage}_events_{i}"
                        )
                        i += 1
