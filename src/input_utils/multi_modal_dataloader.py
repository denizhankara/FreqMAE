...
def create_dataloader(dataloader_option, args, batch_size=64, workers=5):
    ...
    # use base path 
    # base_path = args.dataset_config['base_path']
    # index_file = os.path.join(base_path, index_file)
    # init the flags
    ...
    # sequence_sampler_flag = args.sequence_sampler and (args.stage == "pretrain" or args.stage=="finetune") and (args.train_mode == "contrastive" or args.train_mode == "generative") # This doesnt make much sense, but fix later

    # init the dataset
    if sequence_sampler_flag:
        print("=\tUsing sequence sampler for training")
